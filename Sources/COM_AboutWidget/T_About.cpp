    ...
    : ElaWidget (parent)
{
    ...
    pixCard->setCardImage (QImage (QString (":/Sources/icon/splash.png").replace ("splash.png",
                                                                          eTheme->getThemeMode () == ElaThemeType::Dark ? "splash_dark.png" : "splash_light.png")));

    // 主题变化时更新图标
    connect (eTheme, &ElaTheme::themeModeChanged, [=]() {
        QString pngLo = QString(":/Sources/icon/splash.png").replace("splash.png",
                                                                 eTheme->getThemeMode() == ElaThemeType::Dark ? "splash_dark.png" : "splash_light.png");
    ...
    wxpay->setPixmap (QPixmap (":/Sources/icon/wxpay-removebg-preview.png"));
    wxpay->setScaledContents (true);
    wxpay->setFixedSize (180, 180); // 增大二维码尺寸

    ElaText *alipay = new ElaText (this);
    alipay->setWordWrap (false);
    alipay->setPixmap (QPixmap (":/Sources/icon/alipay-removebg-preview.png"));
    ...
    for (int i = 0; i < payLayout->count(); ++i) {
        QLayoutItem* item = payLayout->itemAt(i);
        if (item->widget()) {
            item->widget()->hide();
        }
    }
